<template>
  <div class="space-y-8">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
      <div>
        <h1 class="text-3xl font-bold font-display text-white">Reportes de Ventas & Exportación PDF</h1>
        <p class="text-xs text-slate-400 font-mono mt-1">Genera reportes corporativos oficiales y analiza el rendimiento comercial</p>
      </div>

      <button
        @click="downloadPDF"
        :disabled="downloading"
        class="px-6 py-3 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-dark-900 font-bold text-sm glow-btn flex items-center gap-2 shadow-xl shadow-emerald-950/50"
      >
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div class="glass-panel p-6 rounded-2xl border border-slate-800">
        <span class="text-xs font-mono text-slate-400 uppercase">Ingresos por Ventas</span>
        <div class="text-3xl font-bold font-display text-emerald-400 mt-2">
          ${{ analytics?.metrics?.totalRevenue?.toFixed(2) || '0.00' }}
        </div>
        <p class="text-[11px] text-slate-400 mt-1">Total acumulado de órdenes pagadas</p>
      </div>

      <div class="glass-panel p-6 rounded-2xl border border-slate-800">
        <span class="text-xs font-mono text-slate-400 uppercase">Promedio por Pedido</span>
        <div class="text-3xl font-bold font-display text-white mt-2">
          ${{ averageTicket.toFixed(2) }}
        </div>
        <p class="text-[11px] text-slate-400 mt-1">Valor promedio de compra por cliente</p>
      </div>

      <div class="glass-panel p-6 rounded-2xl border border-slate-800">
        <span class="text-xs font-mono text-slate-400 uppercase">Tasa de Conversión Stock</span>
        <div class="text-3xl font-bold font-display text-purple-400 mt-2">
          {{ stockConversionRate }}%
        </div>
        <p class="text-[11px] text-slate-400 mt-1">Productos con rotación activa</p>
      </div>
    </div>

    <!-- Chart -->
    <div class="glass-panel p-6 rounded-2xl border border-slate-800 space-y-4">
      <h3 class="text-lg font-bold font-display text-white">Análisis de Tendencia de Ingresos</h3>
      <SalesChart :labels="chartLabels" :data="chartData" title="Ingresos ($ USD)" />
    </div>

    <!-- Top Products Table -->
    <div class="glass-panel p-6 rounded-2xl border border-slate-800 space-y-4">
      <h3 class="text-lg font-bold font-display text-white">Top 5 Productos Más Vendidos</h3>
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-xs font-mono text-slate-300">
          ...
          <tbody class="divide-y divide-slate-800">
            <tr v-for="prod in analytics?.topProducts" :key="prod.id" class="hover:bg-slate-900/50">
              <td class="p-3 font-bold text-white">{{ prod.name }}</td>
              <td class="p-3 text-slate-400">{{ prod.category }}</td>
              <td class="p-3 text-center font-bold text-brand-400">{{ prod.totalQuantity }} uds.</td>
              <td class="p-3 text-right font-bold text-emerald-400">${{ prod.totalRevenue.toFixed(2) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
